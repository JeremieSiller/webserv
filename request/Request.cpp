...
/*   Updated: 2022/03/17 18:14:06 by nschumac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int Request::_parseHeader()
{
	...
	begin = pos;
	pos = this->_header.find(begin + 1, CRLF);
	if (pos == std::string::npos)
	...
	while (true)
	{
		...
		if (pos == std::string::npos)
		{
			pos = this->_header.find(begin + 2, CRLF);
			if (pos == std::string::npos)
				return 0;
			break;
		}
	...
	if (this->_parseHeader.count("Content-Type"))
	{
